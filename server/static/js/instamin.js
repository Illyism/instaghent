...
function init() {
    ...
    if (instaghent.sortingMethod == "author") {
        ...
        if (instaghent.timeFrame) {
            $.getJSON("/more/"+instaghent.timeFrame+"/"+instaghent.sortingMethod+"/0", null, function(data, status) {
                photo_array = data;
...
function update() {
    if (instaghent.sortingMethod == "author") {
        ...
        if (instaghent.timeFrame) {
            $.getJSON("/more/"+instaghent.timeFrame+"/"+instaghent.sortingMethod+"/"+offset, null, function(data, status) {
                $('<div class="picrow"></div>').appendTo("#pics")
                photo_array = photo_array.concat(data);
                f();
                offset += 10;
